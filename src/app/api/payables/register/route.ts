...
import { cancelPayable, createPayable } from "@/app/app/[id]/payables/actions";
...
export async function POST(request: Request) {
  try {
    ...
    if (invoice && invoice.type !== "application/pdf") return NextResponse.json({ error: "Solo se permiten archivos PDF" }, { status: 400 });
    ...
    const res = await createPayable(condominiumId, general);
    if (res?.error) return NextResponse.json({ error: res.error }, { status: 400 });
    const payableId = (res as any).payableId;
    if (!payableId) return NextResponse.json({ error: "No se pudo crear la OP" }, { status: 500 });
    ...
    if (invoice) {
      ...
      if (!uploadError) {
        ...
        console.error("Error subiendo factura OP", uploadError);
      }
    }

    return NextResponse.json({ success: true, payableId });
  } catch (e: any) {
    console.error("Error registrando OP", e);
    return NextResponse.json({ error: e?.message || "Error registrando OP" }, { status: 500 });
  }
}
