...
export async function getCondominiumInfo(condominiumId: string) {
  ...
  return {
    ...data,
    ruc: data.fiscal_id || null, // Mapear fiscal_id a ruc
    property_type: data.type || null, // Mapear type a property_type
    phone: null, // No existe en la BD, pero el formulario lo espera
    administrator_name: null, // No existe en la BD, pero el formulario lo espera
  };
}
...
export async function updateCondominiumInfo(condominiumId: string, formData: FormData) {
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      fiscal_id: result.data.ruc || null, // Mapear ruc a fiscal_id
      address: result.data.address || null,
      type: result.data.property_type || null, // Mapear property_type a type
      logo_url: result.data.logo_url || null,
      // Nota: phone y administrator_name no existen en la tabla condominiums
    })
    .eq("id", condominiumId)
    .select()
    .single();
