...
export async function getUnitsForCharges(condominiumId: string) {
  ...
  if (unitIds.length > 0) {
    ...
    (contactos || []).forEach((c: { unit_id: string; profile?: { full_name?: string | null } | null }) => {
      contactosMap.set(c.unit_id, c.profile?.full_name || "");
    });
  }
...
export async function eliminarCargo(condominiumId: string, chargeId: string) {
  const supabase = await createClient();

  // Si tiene pagos, no permitir borrar
  const { data: alloc } = await supabase
    .from("payment_allocations")
    .select("id")
    .eq("charge_id", chargeId)
  ...
  if (error) return { error: "No se pudo borrar el cargo." };

  revalidatePath(`/app/${condominiumId}/charges`);
  return { success: true };
}

export async function eliminarCargosMasivos(condominiumId: string, chargeIds: string[]) {
  if (!Array.isArray(chargeIds) || chargeIds.length === 0) return { error: "Selecciona al menos un cargo." };
  const supabase = await createClient();

  // Identificar cargos con pagos
  ...
  if (idsSinPago.length > 0) {
    await supabase.from("charges").delete().in("id", idsSinPago).eq("condominium_id", condominiumId);
  }

  revalidatePath(`/app/${condominiumId}/charges`);
  return {
    success: true,
    borrados: idsSinPago.length,
