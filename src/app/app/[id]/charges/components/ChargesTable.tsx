  ...
  const handleDeleteSelected = () => {
    ...
    startTransition(async () => {
      ...
      if (res?.error) alert(res.error);
        ...
        alert(`Borrados: ${res.borrados}. No se borraron ${res.bloqueados} porque tienen pagos.`);
      }
    });
  };
          ...
          {charges.map((c) => (
            <tr key={c.id} className="hover:bg-gray-50/70">
              ...
              <td className="px-3 py-3 text-gray-800">
                {c.unit?.unit_contacts?.[0]?.profiles?.full_name ? `👤 ${c.unit.unit_contacts[0].profiles.full_name}` : "—"}
              </td>
